      ...
      XXXstatePlugins: {
        example: {
          actions: {
            ...
            oneOfModeSelection: (e) => {
                return {
                    type: 'ONEOF_MODE',
                    mode: e.target.dataset.name
                };
            }
          },
          reducers: {
            'ONEOF_INDEX': (state, action) => {
                return state.set('oneOfModelIndex_'+ action.key, action.index);
            },
            'ONEOF_MODE': (state, action) => {
                return state.set('oneOfMode', action.mode);
            }
          },
          selectors: {
            oneOfModelSelection: (state, key, maxIndex) => {
                var oneOfModelIndex = state.get('oneOfModelIndex_'+ key) ;
                return oneOfModelIndex ? oneOfModelIndex : 0;
            },
            oneOfModeSelection: (state) => state.get('oneOfMode') ? state.get('oneOfMode') : 'FIRST'
          }
        }
      },
      wrapComponents: {
